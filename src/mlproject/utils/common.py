...
def create_directories(path_to_directories: list, verbose = True):
  ...
  Args:
    ...
    ignore_log(bool, optional): ignore if multiple dirs is to be created. Defauult to False
  """
...
def save_bin(data: Any, path: Path):
  ...
  Args:
    ...
    path(Path): path to save the binary
  """
  joblib.dump(value=data, filename=path)
  logger.info(f" binary file successfully at: {path}")
...
def load_bin(path: Path) -> Any:
  ...
  Returns:
    Any: object representing the binary
  """
  data = joblib.load(path)
  logger.info(f" binary file successfully from: {path}")
  return data
